...
Set default values.
*/}}
{{- define "cn.defaultValues" }}
{{- if not .defaultValuesSet }}
  {{- $name := include "cn.fullname" . }}
  {{- include "cn.requiredValues" . }}
  {{- with .Values }}
...
app.kubernetes.io/component: connect-node
{{- end }}
...
*/}}
{{- define "cn.secretNames" -}}
{{- $secrets := list }}
  {{- with .Values.config.creds }}
    {{- if .secretName }}
      {{- $secrets = append $secrets (merge (dict "name" "creds") .) }}
    {{- end }}
  {{- end }}
